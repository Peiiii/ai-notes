...
const ExplorationCard: React.FC<{ exploration: Exploration }> = ({ exploration }) => {
    ...
    const handleRetry = () => {
        // We assume the parentId and sourceNoteId are on the failed wikiEntry stub
        // This might not be robust if entry creation fails early. A more robust retry
        // would store parentId/sourceNoteId on the exploration object itself.
        // For now, this handles regeneration failures well.
        if (exploration.wikiEntry?.parentId && exploration.wikiEntry?.sourceNoteId) {
            ...
            presenter.handleDismissExploration(exploration.id); // Remove the failed one
        } else {
             // A more generic retry if we don't have context, assuming root-level exploration
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 p-3 flex items-center justify-between gap-2 animate-in fade-in slide-in-from-bottom-5 duration-300">
            {renderContent()}
...
const ExplorationTray: React.FC = () => {
    ...
    const PanelWrapper: React.FC<{children: React.ReactNode}> = ({children}) => {
        ...
        return <div className="fixed bottom-4 right-4 w-full max-w-sm z-50">{children}</div>
    }

    return (
        <PanelWrapper>
            <div className={`p-2 flex items-center justify-between ${isSidebar ? 'border-b border-slate-200 dark:border-slate-700 flex-shrink-0' : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-t-lg -mb-2'}`}>
                ...
                <div className="flex items-center">
                    <button 
                        ...
                        title={isSidebar ? "Collapse to Tray" : "Expand to Sidebar"}
                        className="p-2 rounded-full text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    >
                        {isSidebar ? <Square2StackIcon className="w-5 h-5" /> : <ArrowUpOnSquareIcon className="w-5 h-5" />}
                    </button>
                    <button 
            ...
            <div className={isSidebar ? "flex-1 overflow-y-auto p-2 space-y-2" : "space-y-2"}>
